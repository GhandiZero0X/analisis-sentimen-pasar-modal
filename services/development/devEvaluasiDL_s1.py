...
Catatan:
  ...
  → F1 menggunakan average="binary" untuk metrik overall.

Input  : dev_database/4_model/S1/dl/after/
         best_model.bin
         config.json
         tokenizer/
...
Output : dev_database/4_model/S1/dl/after/
...
MODEL_DIR = BASE_DIR / "dev_database" / "4_model" / "S1" / "dl" / "after"
...
for f in [MODEL_BIN_FILE, ENCODER_FILE, SPLIT_FILE, TOKENIZER_DIR]:
    if not Path(f).exists():
        raise FileNotFoundError(
            f"\nFile tidak ditemukan: {f}\n"
            "Pastikan model dan artefak training sudah lengkap."
        )
...
if "positif" in class_names:
    pos_label = class_names.index("positif")
else:
    pos_label = 1 if n_classes > 1 else 0

print("✅ Artefak dimuat")
...
if TRAIN_LOG_FILE.exists():
    ...
    print("⚠️  train_log.csv tidak ditemukan, training curve dilewati")
