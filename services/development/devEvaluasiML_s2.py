...
Catatan:
  Model dilatih dengan 2 kelas: positif & negatif (tanpa netral)
  → AUC-ROC menggunakan binary roc_curve (bukan One-vs-Rest)
  → y_score dari decision_function berbentuk 1D (n_samples,)

Input  : dev_database/4_model/S2/ml/after/    ← ✅ S2 bukan S1
         svm_model.joblib
         label_encoder.joblib
         X_test.joblib
         y_test.joblib

Output : dev_database/4_model/S2/ml/after/
...
MODEL_DIR = BASE_DIR / "dev_database" / "4_model" / "S2" / "ml" / "before"
...
print("  Skenario 2: tanpa kelas netral")
print("=" * 60)
...
for f in [MODEL_FILE, ENCODER_FILE, X_TEST_FILE, Y_TEST_FILE]:
    if not f.exists():
        raise FileNotFoundError(
            ...
            "Jalankan devModellingML.py (Skenario 2) terlebih dahulu."
        )
...
ax.set_title(
    f"Confusion Matrix — SVM LinearSVC (S2)\nAccuracy: {acc:.4f}",
    fontsize=13, pad=15
)
...
ax.set_title(f"ROC Curve — SVM LinearSVC (S2)\nAUC = {roc_auc:.4f}", fontsize=13)
...
report_text = f"""EVALUATION REPORT — SVM LinearSVC (Skenario 2 — Binary)
=========================================================
...
eval_info = f"""EVALUATION INFO — SVM LinearSVC (Skenario 2)
=============================================
Tanggal        : {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
Skenario       : 2 (tanpa kelas netral)
Test set       : {X_test.shape[0]:,} baris
...
# ══════════════════════════════════════════════════════════════
print(f"\n{'='*60}")
print(f"  ✅ Evaluasi selesai! (Skenario 2)")
print(f"{'='*60}")
