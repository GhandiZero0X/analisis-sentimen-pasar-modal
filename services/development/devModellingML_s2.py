...
Scenario 2:
    - Hanya label POSITIF dan NEGATIF yang dimasukkan ke model karena data hanya 2 label saja

Catatan:
    Imbalance ditangani dengan pendekatan HYBRID (sama seperti Skenario 1):

      1) SMOTE (data-level)
         Representasi TF-IDF berupa vektor numerik kontinu, sehingga
         interpolasi linear antar sampel tetangga (prinsip kerja SMOTE)
         valid secara matematis. Diterapkan SETELAH TF-IDF fit_transform
         dan HANYA pada data train (setelah split, sebelum training).
         Test set TIDAK ikut di-SMOTE, supaya tetap merepresentasikan
...
Input  : dev_database/3_preprocessing/S2/ml/
         tweets_covid_labellingLexicon_preprocessingML.csv

Output : dev_database/4_model/S2/ml/covid/
         svm_model.joblib
...
INPUT_DIR  = BASE_DIR / "dev_database" / "3_preprocessing" / "S2" / "ml"
OUTPUT_DIR = BASE_DIR / "dev_database" / "4_model" / "S2" / "ml" / "covid"
...
# ══════════════════════════════════════════════════════════════
print("=" * 60)
print("  TRAINING SVM - TF-IDF + LinearSVC + GridSearchCV (SKENARIO 2)")
...
# Hanya ambil label positif dan negatif
df_all = df.copy()
...
if len(df) == 0:
    raise ValueError("Tidak ada data positif/negatif setelah filtering.")
...
if len(le.classes_) != 2:
    raise ValueError(f"Scenario 2 harus 2 kelas, tetapi ditemukan: {list(le.classes_)}")
...
# Binary F1 di test set (test set TIDAK di-SMOTE, tetap distribusi asli)
pos_label = list(le.classes_).index("positif")
test_f1 = f1_score(y_test, y_test_pred, average="binary", pos_label=pos_label, zero_division=0)
...
# ══════════════════════════════════════════════════════════════
info = f"""
TRAINING INFO - SVM LinearSVC (Scenario 2, Hybrid Imbalance Handling)
==========================================
