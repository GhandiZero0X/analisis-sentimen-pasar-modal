# services/development/devModellingDL_s2.py
"""
=============================================================
STEP 4a: FINE-TUNING IndoBERTweet (BINARY) — SKENARIO 2
=============================================================
...
Input  : dev_database/3_preprocessing/S2/dl/
         tweets_all_periods_labellingLexicon_preprocessingDL.csv

Output : dev_database/4_model/S2/dl/all_periods/
         best_model.bin
         config.json
         tokenizer/
...
OUTPUT_DIR = BASE_DIR / "dev_database" / "4_model" / "S2" / "dl" / "covid"
...
def train_epoch(model, dataloader, optimizer, scheduler, device, loss_fn):
    ...
    terhadap outputs.logits. Model dipanggil TANPA argumen `labels`,
    supaya model hanya mengembalikan logits mentah -- loss sepenuhnya
    dikontrol sendiri lewat loss_fn yang sudah diberi bobot kelas.
    """
    model.train()
...
def main():
    ...
    print("  SKENARIO 2 - HYBRID IMBALANCE HANDLING")
    print("=" * 62)
    print(f"\nInput  : {INPUT_FILE}")
    ...
    info = f"""TRAINING INFO - IndoBERTweet Fine-tuning (Skenario 2)
=====================================
...
"""
    ...
    print(f"{'='*62}")
    print(f"\n  -> Lanjut ke: python devEvaluasiDL_s2.py")
