...
// Siapkan Akun untuk scrapping login via google
// sebelum di gunakan pastikan nama untuk hasil scrapping sudah di sesuaikan 
// data kolom yang di hasilkan adalah : date, tweet, sentiment, saham sesuaikan saja dengan kebutuhan data yang di inginkan, 
// untuk kolom sentiment bisa di isi dengan hasil analisis sentimen dari library sentiment atau bisa juga di isi dengan manual sesuai kebutuhan
...
const twitterURLs = [
    // cara pengguanaan kata kunci menggunakan url search di x.com (twitter) 
    // dengan format: https://x.com/search?q=KEYWORD%20lang%3aID%20until%3AEND_DATE%20since%3ASTART_DATE&src=typed_query&f=live
    // katakunci code dapat menjalan labih dari 1 kata kunci jika sudah tidak di gunakan dapat di komenatau di hapus saja
    // contoh penggunaan :
...
async function scrapeTweets() {
    ...
    if (fs.existsSync("sesuaikan_nama_file.json")) {
        const existing = JSON.parse(fs.readFileSync("sesuaikan_nama_file.json", "utf-8"));
    ...
    fs.writeFileSync("sesuaikan_nama_file.json", JSON.stringify(tweetArray, null, 2));
